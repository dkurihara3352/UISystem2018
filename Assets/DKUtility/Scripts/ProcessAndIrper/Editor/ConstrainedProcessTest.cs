...
public class ConstrainedProcessTest {
	...
	public void Construction_DiffThresholdIsLessThanZero_ThrowsException(float diffThreshold){
		...
		TestConstrainedProcess testProcess = new TestConstrainedProcess(processManager, ProcessConstraint.none, 0f, diffThreshold, 0f, null);
	}
	[Test]
	public void Construction_DiffThresholdZero_DoesNotThrowException(){
		float diffThreshold = 0f;
		IProcessManager processManager = Substitute.For<IProcessManager>();
		Assert.DoesNotThrow(() =>{ new TestConstrainedProcess(processManager, ProcessConstraint.none, 0f, diffThreshold, 0f, null);
		});
	}
	[Test]
	public void Construction_ProcessManagerNull_ThrowsException(){
		Assert.Throws(Is.TypeOf(typeof(System.ArgumentException)).And.Message.StringContaining("process never works without processManager"), () => {
			new TestConstrainedProcess(null, ProcessConstraint.none, 0f, 0f, 0f, null);
		});
	}
	...
	public class TestConstrainedProcess: AbsConstrainedProcess{
		public TestConstrainedProcess(IProcessManager processManager, ProcessConstraint processConstraint, float constraintValue, float diffThreshold, float normalizedValueDiff, IWaitAndExpireProcessState state): base(processManager, processConstraint, constraintValue, diffThreshold, state){
	...
	public TestConstrainedProcess CreateTestConstrainedProcess(float normalizedValueDiff, float diffThreshold){
		IProcessManager processManager = Substitute.For<IProcessManager>();
		TestConstrainedProcess testProcess = new TestConstrainedProcess(processManager, ProcessConstraint.none, 0f, diffThreshold, normalizedValueDiff, null);
		return testProcess;
	}
	public TestConstrainedProcess CreateTestConstrainedProcess(bool valueDiffIsBigEnough, ProcessConstraint processConstraint, float constraintValue){
		IProcessManager processManager = Substitute.For<IProcessManager>();
		TestConstrainedProcess testProcess = new TestConstrainedProcess(processManager, processConstraint, constraintValue, .5f, valueDiffIsBigEnough? 1f: 0f, null);
		return testProcess;
	}
	...
	public TestConstrainedProcess CreateTestConstrainedProcess(float expireT, out IWaitAndExpireProcessState processState, out IProcessManager processManager){
		...
		TestConstrainedProcess testProcess = new TestConstrainedProcess(procMan, ProcessConstraint.expireTime, expireT, .5f, 1f, procSta);
		processState = procSta;
